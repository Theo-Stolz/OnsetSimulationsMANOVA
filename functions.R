...
permtdist <- function(cond1, cond2, Nf, Nt, nboot = 2000){
  ...
  all <- Rfast::transpose(cbind(cond1, cond2)) # combine trials, then transpose -> trials x time points
...
permtdist.elec <- function(cond1, cond2, Ne, Nf, Nt, nboot = 2000){
  ...
  for(B in 1:nboot){
    ...
    for(E in 1:Ne){
    perm.tvals[E,,B] <- Rfast::ttests(Rfast::transpose(permcond1[E,,]), Rfast::transpose(permcond2[E,,]), paired=FALSE)[,1]    
    }
  }
  perm.tvals
}
